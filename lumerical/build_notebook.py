...
4. 并行配置：混合架构 CPU（4P+4E+4LPE）上加 MPI 进程反而变慢——12 进程比 6 进程慢约 4.5 倍（每步被最慢的 LP-E 核拖住，早期误以为"丢 engine"）；实测最优为 6 进程 × 2 线程（`setresource("FDTD", 1, "processes", 6)` + `threads=2`），比 6×1 快约 18%（见 `verify/v5_fdtd_scaling.py` 与 `results/verify/fdtd_scaling.json`）；全大核平台（如 7945HX）建议用 `FDTD_PROCESSES=12 FDTD_THREADS=1`，比 6 进程快约 37%；
