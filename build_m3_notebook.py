# -*- coding: utf-8 -*-
"""Build milestone-M3-delay-learning-and-budget.ipynb."""
import io
import json
...
md("""# 里程碑 M3：自主提问的仿真（delay learning 的价值边界 + 物理延迟预算）
...
md("""## 1. Delay learning 的价值边界（三个实验）
...
d = np.load("lidar-pointnet/snn/outputs_classify/delay_learning_results.npz")
...
md("""**发现 1**：空间任务上可学习延迟几乎无增益（0.838 → 0.841）。
...
code("""d2 = np.load("lumerical/results/m3b_delay_budget.npz")
...
with io.open("milestone-M3-delay-learning-and-budget.ipynb", "w", encoding="utf-8") as f:
    json.dump(nb, f, indent=1, ensure_ascii=False)
print("wrote milestone-M3 with %d cells" % len(cells))
