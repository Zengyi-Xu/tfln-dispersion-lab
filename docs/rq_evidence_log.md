...
## 2026-09-26 第七十七批：批 73 基准表勘误+扩三行（数字对手盘一手溯源）

- [勘误·我方批 73] "多板 FPGA CMOS 退火机 1.3M 自旋（9 板×16k/片）"**混了两个源**：1.3M=Yamamoto/Kawahara 2022（Microprocessors and Microsystems 95, 104674，Aadhi ref 169，东京理科大河原组全耦合退火处理系统）；"9 板×16k 自旋/片"是 Aadhi 另一段的多板 SBM 架构描述。表已更正。
- [扩行] ① Kawahara 组 CMOS 芯片族：Takemoto 2021（3×16k=49k 自旋，**对 CPU 快 233×/能耗低 972× 实测**——ASIC vs CPU 口径的能耗锚点）→ 2024 年 36×22nm LSI=147k 自旋（4096/LSI）；② SBM 行补正式引文（Tatsumura Nat. Electron. 4, 208 (2021)；Kashimata IEEE Access 12, 36606 (2024)）；③ 新增**百万 p-bit 概率计算机**（arXiv:2606.25313，2026-06 预印本）——数字/物理混合对手也到百万级，"光子赢在能效不赢在规模"论据继续加固。
- [判断] 数字对手盘 2026 年全景：FPGA 1.3M / CMOS ASIC 147k+233×加速 / p-bit 1M / GPU 集群数十 kW——规模赛道彻底无望，能效-延迟-集成度是唯一开口，与既有定位一致。
...
## 2026-09-26 第七十九批：rc_tutorial §六/§七 与批 52–78 对齐检查

- [§七.5] "08b 加难档在跑"过期 → 更新为 08b ✅（noise≤0.2 领先/能耗 9.0–10.5×/N=64 下限，口径指针 §六.4）。
- [§七.6] 结构化伊辛补仿真链闭环状态：09 ✅ 369/369 + sim10 ✅ 56/56 四条验收级结论一行化。
- [§七.2] 频域复用项补 sim11 已下发+标定头条（GD 0.068 vs 随机 0.126=2×，标注待全量验收）。
- [§六.5] "商业化没有 RC"补 Roadmap 2025 旁证（130+作者自承"电信主导用例太少"=生态问题非原理问题）。
- [核查] §六其余各项（vs LSTM/NGRC/OSP/SNN/Catch-22）与批 52–78 无冲突；§一~§四不动。
