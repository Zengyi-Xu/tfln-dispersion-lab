...
#### 数字对手盘基准表（Aadhi 2026 全文挖掘，rq 第七十三批；引用纪律：区分实测/估计）

| 对手 | 规模 | 功耗/成本 | 口径 |
|---|---|---|---|
| 25×NVIDIA H100 集群 | 百万变量 MVM | **数十 kW、数百万美元**；同步延迟/TB 级存储/冷却是部署主障碍 | 综述引 NVIDIA 官方架构数据（能力估计） |
| 多板 FPGA 全耦合退火处理系统 | **1.3×10⁶ 自旋**（Yamamoto/Kawahara 2022, Microprocessors and Microsystems 95, 104674=Aadhi ref 169） | 多板系统通信开销主导计算 | 已演示（**rq 77 勘误**：批 73 误并"9 板×16k/片"——那是 Aadhi 另一段的多板 SBM 架构，非本系统） |
...
- 其他可引用点：① sigmoid 非线性比多项式 TTS 好一个数量级（ref 14，呼应 06 的
  sin² 选择）；② 光纤方案 >10⁵ 自旋不稳、实际 <5 km；③ 其 Fig 1(d) 蜘蛛图=
  CMOS vs 光子五维对照（scalability/reliability/speed/reconfigurability/footprint）
  现成框架。全文转文本存 `.tmp/aadhi2026.txt`。
- **写法弹药**：「25 块 H100 做百万变量 MVM 要数十 kW 与数百万美元（Aadhi 2026）；
  我们不主张在规模上对抗 CMOS/GPU，而主张把**耦合存进色散几何**后单芯片
  W 级功耗完成结构化耦合层——能效而非规模是我们的赛道」。
...
- **Mohseni, McMahon & Byrnes, Nat. Rev. Phys. 4, 363 (2022)（被引 881，领域标准
  ...
  我们因此不主张优化霸权，而主张 XX」。
- **Jiang/Shu/Lin, IEEE Access 12, 125014 (2024)（被引 22，台湾中央大学 CS 组=
  无光子利益方的第三方基准，全文已读，rq 第七十四批）**：D-Wave Advantage(QA)/
  Fujitsu DAU-3(DA)/Compal GPU 退火机(GPUA) × 8 类 COP × 经典 SOTA 算法对打。
  **执行时间榜首计数：CA 42 / DA 13 / QA 2 / GPUA 1**——连商用退火机三巨头在
  经典基准上都极少赢下墙钟时间；QA 的长耗时主因是分解+嵌入开销
  （EnergyImpactDecomposer 的映射税）。其结论口径与 Mohseni 完全互洽：
  退火机的机会在"CA 尚未深耕的新问题"，不是已解决的基准。**方法论可引用点**：
  解质量与执行时间双轴排名 + X/IS/N/A 情形处理协议（我们 06/06b/09/10 的
  success/tts 口径与之兼容）；**我方引用句式**：「第三方基准显示连最佳商用
  退火机在经典 COP 上的执行时间榜首率也仅 13/58（Jiang et al. 2024）——
  优化基准不是物理新硬件的证明场，能效/延迟/集成度才是」。
  PDF 存 `.tmp/jiang2024.pdf`+`.txt`。
...
8. **色散 GD 作伊辛耦合介质**（2026-09-26 新增，交叉检验完成）：思路=自旋按
   ...
   - **结论：用连续色散 GD 生成耦合这条路没人走过，但物理上有个硬约束必须
     ...
     Max-Cut。定位：叙事应是"结构化伊辛（物理启发的格点模型）的天然硬件"，
     不与 WDM 全可编程伊辛机正面竞争。**状态更新（rq 第七十六批）**：仿真 09（碰撞调度器）✅ 369/369
     验收、sim10（共享增益混频结构化伊辛）✅ 56/56 验收（单调 XGM 混频器=
     正贡献、GDR 几乎无损 ≤1.5%、增益定标纪律对象=非单调性而非摆幅、
     09 拟合核可用）——两段式架构仿真链已闭环，详细数字见
     `sim10_design_draft.md` §11。
...
9. **微梳 RC 的模式间延迟硬件化**（rq 第七十/七十二批新增，sim11 已下发）：
   Cuevas/Kuse 2025 实验证明模式间延迟多样性是微梳频率复用 RC 最强单一杠杆
   （Santa Fe NMSE 0.8→0.21），但只能后处理 trial-and-error——他们没有片上
   模式相关延迟器件。**我们的啁啾光栅 GD 恰是确定性、可设计的波长相关延迟
   元件**：把他们的后处理技巧变成硬件层可工程化旋钮。sim11 **全量验收值**
   （rq 第八十一批，556 行 @ 7945HX，五条标准全过，RECEIPT 已回）：确定性
   线性 GD（C 臂 best）在 santafe 全部四符号率档大幅低于随机延迟试错上限
   （B 臂 best）——50M: **0.019 vs 0.148**；200M: 0.013 vs 0.209；
   1G: 0.128 vs 0.327；4G: 0.622 vs 0.655；Cuevas 方向复现
   （A=0.986 → B best=0.148，定性同其实验 0.8→0.21）；narma 负面对照成立
   （C 不优于 B，GD 增益是任务特异的）。⚠️ 诚实注解：最优 C 点 GD 斜率
   36–365 ps/nm 全超 100 GHz 间隔下 10 ps/nm 保守工程锚点，工程子集内
   C 未超 B——表述必须条件式。物理映射注意：a=100 ps/mode 对应
   D=125 ps/nm @100 GHz 间隔（超工程可达 10 ps/nm），**但换 1 THz 间隔梳
   所需 D 降 10× 即落入可达域**——"GD 延迟 × 模式间隔 × 符号率"三元设计
   空间是这个切口的完整故事。定位=盟友切口非对抗：引用 Cuevas 为需求证明，
   我们供硬件化路径。
