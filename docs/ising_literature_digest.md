...
第 1 行对我们的威胁解除。**剩余真风险只有 Kong 2026**（时-波长耦合虚拟
节点的具体实现待全文确认）和 PIC-OPO RC（χ² 高速 RC 的路线占位）。

**贴脸的是 Kong 2026**：「时-波长耦合虚拟节点」与我们 CBG 的
"τ_r(λ) 波长复用延迟抽头"物理直觉高度相似。**在拿到全文之前，
我们的差异化假设（待证）**：① 他们若用微环/WDM 抽头而非啁啾光栅的
**连续色散 GD 整形**，则"色散工程作为计算资源"仍是我们的领地；
② hybrid RC-NGRC 片上化（§三-11）无人占位；③ Ising+RC 统一平台
无人占位；④ spike 域端到端信号链（lidar→spike→RC）无人占位。
