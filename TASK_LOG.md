...
- **重任务外发约定（2026-09-26 起）**：预计运行 >30 min 的仿真/计算任务，写成任务书 md（目标、脚本、参数、验收标准、产物路径）放入同步文件夹 `D:\BaiduSyncdisk\Temp_transit\`，文件名 `TASK_REQUEST_<yyyymmdd>_<名称>.md`；另一台机器有定时 Kimi 任务每 10 分钟检查该文件夹并执行。执行方完成后把结果连同回执 md 放回原文件夹或 push 到仓库分支。小任务（<30 min）本机直接做，不外发
